export async function createCesiumInstance(cesiumContainerElement) {
  ...
  //创建viewer
  const viewer = new Cesium.Viewer(cesiumContainerElement, {
...
export function setDefaultView(viewer){
  // 指定经纬度和高度（单位：米）
...
export function setDefaultPosition(viewer) {
  // 指定经纬度和高度（单位：米）
...
export function addAirportMarker(viewer) {
    // 解构机场数据并设置默认值
    const lon = 113.3768; // 经度
    const lat = 23.107552; // 纬度
    const name='铁塔信号塔机场'
